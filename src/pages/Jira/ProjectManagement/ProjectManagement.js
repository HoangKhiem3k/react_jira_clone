...
import { Table, Tag, Space, Button, Popconfirm, Avatar, Popover, AutoComplete } from 'antd';
// import ReactHtmlParser from "html-react-parser";
import { useSelector, useDispatch } from 'react-redux';
import { ADD_USER_PROJECT_API, DELETE_PROJECT_SAGA, EDIT_PROJECT, GET_LIST_PROJECT_SAGA, GET_USER_API, OPEN_FORM_EDIT_PROJECT, REMOVE_USER_PROJECT_API } from '../../../util/constants/settingSystem';
            ...
            render: (text, record, index) => {
                return <div>
                    {record.members?.slice(0, 3).map((member, index) => {
                        return <Popover key={index} placement='top' title='Membesr' content={() =>{
                            return <table className='table'>
                                ...
                                <tbody>
                                    {record.members?.map((item,index) =>{
                                        return <tr key={index}>
                                            <td>{item.userId}</td>
                                            <td><img src={item.avatar} style={{borderRadius:'15px'}} width='30' height='30' alt=''/></td>
                                            <td>{item.name}</td>
                                            <td>
                                                <button onClick={() =>{
                                                ...
                                                }} style={{borderRadius:'50%'}} className="btn btn-danger">X</button>
                                            </td>
                                        </tr>
                                    })}
                                </tbody>
                    ...
                    <Popover placement="rightTop" title={"Add user"} content={()=> {
                        ...
                        style={{width:'100%'}} onSearch={(value) => {
                            // debounce search : tranh call nhieu api
